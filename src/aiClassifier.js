class AIClassifier {
    ...
    async classifyTextAnswer(question, answer, availablePaths) {
        ...
        const prompt = this.buildClassificationPrompt(question, answer, availablePaths);
        ...
        try {
            ...
            const response = await fetch('https://api.openai.com/v1/chat/completions', {
                ...
                body: JSON.stringify({
                    ...
                    messages: [
                        {
                            role: 'system',
                            content: 'You are an AI fragrance consultant who classifies user responses into personality-based fragrance categories. You must respond with ONLY the path ID, nothing else.'
                        },
            ...
            if (question.id === 'Q1') {
                ...
                if (answerLower.includes('love') || answerLower.includes('romantic') || answerLower.includes('intimate'))
                    return 'PathA_Choice';
                if (answerLower.includes('confident') || answerLower.includes('bold') || answerLower.includes('power'))
                    return 'PathB_Choice';
                if (answerLower.includes('calm') || answerLower.includes('peace') || answerLower.includes('nature'))
                    return 'PathC_Choice';
                if (answerLower.includes('happy') || answerLower.includes('joy') || answerLower.includes('fun'))
                    return 'PathD_Choice';
                return 'PathE_Choice';
            }
            return availablePaths[0];
    ...
    buildClassificationPrompt(question, answer, availablePaths) {
        const pathDescriptions = {
            'PathA_Choice': 'Sensual & Romantic - for users who want intimate, romantic, sensual fragrances',
            'PathB_Choice': 'Confident & Bold - for users who want powerful, assertive, commanding fragrances',
            'PathC_Choice': 'Calm & Grounded - for users who want peaceful, natural, balanced fragrances',
            'PathD_Choice': 'Joyful & Playful - for users who want happy, fun, energetic fragrances',
            'PathE_Choice': 'Mysterious & Unique - for users who want unusual, complex, intriguing fragrances',
            ...
            'A2_3_Adventure': 'Adventurous Escape - exciting, spontaneous romantic moments',
            ...
            'B2_3_Personal': 'Personal Triumph / Solo - individual achievement and self-assurance',
            'C2_1_Nature': 'A walk in nature / Forest - natural, outdoorsy peace',
            'C2_2_CozyHome': 'A quiet corner of my home - domestic, comfortable tranquility',
            'C2_3_ByWater': 'Near the ocean or a lake - water-based serenity',
            'E2_1_Intellect': 'An intellectual secret / Ancient knowledge - scholarly, mysterious wisdom',
            'E2_2_Forbidden': 'A forbidden, dark secret - edgy, dangerous intrigue',
            'E2_3_Whimsical': 'A whimsical, magical secret - playful, fantastical mystery'
        };
...
Respond with ONLY the path ID (e.g., "PathA_Choice" or "A2_1_Elegant").`;
    ...
    async classifyForFinalResult(userAnswers, availableFinals) {
        ...
        if (!this.apiKey) return availableFinals[0];

        const finalDescriptions = {
            'Final_A_1': 'Romantic Floral with Intimate Sillage - soft, close-to-skin romantic scents',
            'Final_A_2': 'Romantic Floral with Moderate Sillage - balanced romantic presence',
            'Final_A_3': 'Romantic Floral with Strong Sillage - bold romantic statement',
            'Final_B_1': 'Bold Leather with Reserved Projection - subtle confident power',
            'Final_B_2': 'Bold Leather with Assertive Projection - noticeable confident presence',
            'Final_B_3': 'Bold Leather with Unmistakable Projection - commanding confident aura',
            'Final_C_1': 'Clean & Musky with Solitary Character - personal peaceful moments',
            'Final_C_2': 'Clean & Musky with Shared Character - peaceful connections with others',
            'Final_D_1': 'Playful Gourmand with Vibrant Energy - energetic joyful presence',
            'Final_D_2': 'Playful Gourmand with Gentle Glow - soft joyful warmth',
            'Final_E_1': 'Unique Incense with Inviting Character - mysterious but approachable',
            'Final_E_2': 'Unique Incense with Distant Character - mysterious and enigmatic'
        };
...
Which profile best matches this user's responses and personality? Respond with ONLY the profile ID.`;
        ...
        try {
            const response = await fetch('https://api.openai.com/v1/chat/completions', {
                ...
                body: JSON.stringify({
                    ...
                    messages: [
                        {
                            role: 'system',
                            content: 'You are an expert fragrance consultant. Analyze the user\'s complete journey and select the most appropriate final fragrance profile. Respond with ONLY the profile ID.'
                        },
                        { role: 'user', content: prompt }
                    ],
                    max_tokens: 50,
            ...
            return availableFinals.includes(finalResult) ? finalResult : availableFinals[0];
        } catch (error) {
    ...
    async generatePersonalizedPoem(userAnswers, finalProfile) {
        ...
        try {
            ...
            const response = await fetch('https://api.openai.com/v1/chat/completions', {
                ...
                body: JSON.stringify({
                    ...
                    messages: [
                        {
                            role: 'system',
                            content: 'You are a masterful fragrance stylist, a literary genius, and a poet extraordinaire. You craft beautiful, evocative poems that capture the essence of a person\'s fragrance journey. You must respond with ONLY the poem.'
                        },
                        { role: 'user', content: prompt }
                    ],
                    max_tokens: 300,
            ...
            if (!response.ok)
                throw new Error(`Poem generation API request failed: ${response.status}`);
